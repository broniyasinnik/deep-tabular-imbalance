import numpy as np
import pytest
from data_utils import minority_data_undersample, label_column, majority_data_undersample
...
@pytest.mark.parametrize("ratio", [0.9, 0.8, 0.5])
def test_majority_data_undersample(ratio, adult):
    train, _, meta, _, _ = adult
    y_train, index = label_column(train, meta)
    _, counts = np.unique(y_train, return_counts=True)
    minority_count = np.min(counts)
    majority_count = np.sum(counts) - minority_count
    data_sample = majority_data_undersample(train, meta, ratio)
    expected_size = minority_count + np.floor(majority_count * ratio)
    assert data_sample.shape[0] == expected_size
